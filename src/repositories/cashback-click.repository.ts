...
export class CashbackTrackingLinkNotFoundError extends Error {
  constructor() {
    super(
      "Active affiliate cashback tracking link was not found",
    );
    this.name =
...
function readPlatform(
  ...
  throw new Error(
    "Cashback click RPC returned an unsupported platform",
  );
}

function mapClickRow(
  row: RecordCashbackClickRow,
): CashbackClickRecord {
  const platform =
    readPlatform(row.platform);
  ...
  if (
    ...
    throw new Error(
      "Cashback click RPC returned invalid data",
    );
  }
...
export async function recordCashbackClickAsync(
  ...
  if (error) {
    ...
    throw new Error(
      `Unable to record cashback click: ${error.message}`,
    );
  }

  const row = (
    data as RecordCashbackClickRow[] | null
  )?.[0];

  if (!row) {
    throw new Error(
      "Cashback click RPC returned no data",
    );
  }
