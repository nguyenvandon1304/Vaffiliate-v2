...
const placeholders: Record<PlatformName, string> = {
  ...
  "TikTok Shop":
    "Dán link sản phẩm từ TikTok Shop tại đây",
};

const invalidMessages: Record<PlatformName, string> = {
  Shopee:
    "Bạn đang chọn Shopee. Vui lòng dán link sản phẩm Shopee hợp lệ.",
  "TikTok Shop":
    "Bạn đang chọn TikTok Shop. Vui lòng dán link sản phẩm TikTok Shop hợp lệ.",
};

const initialActionState: CreateCashbackTrackingLinkActionState =
  {
    success: false,
    message: "",
    trackingLink: null,
...
function isValidDestinationUrl(
...
export default function CashbackForm() {
  ...
  const check = () => {
    ...
    if (!value) {
      setError(
        "Vui lòng dán link sản phẩm trước khi kiểm tra hoàn tiền.",
      );
      return;
    }

    if (
      !isValidDestinationUrl(platform, value)
    ) {
      setError(invalidMessages[platform]);
      return;
    }
  ...
  const resultPlatform =
    actionState.trackingLink
      ? platformLabels[
          actionState.trackingLink.platform
        ]
      : platform;
    ...
    <>
      <div className="rounded-[var(--radius-xl)] border border-[color:var(--line)] bg-[rgba(255,252,249,0.88)] p-5 shadow-[var(--shadow-sm)]">
        ...
        <p className="mt-2 text-sm leading-6 text-[color:var(--text-muted)]">
          Sao chép link sản phẩm từ Shopee hoặc
          TikTok Shop, dán vào đây để Vaffiliate
          tạo link hoàn tiền cho bạn.
        </p>
            ...
            {(
              ...
              return (
                <button
                  ...
                  className={`rounded-[var(--radius-lg)] border px-4 py-3 text-sm font-semibold transition-all disabled:cursor-not-allowed disabled:opacity-60 ${
                    active
        ...
        <div className="mt-4">
          ...
          <input
            ...
            autoComplete="off"
            inputMode="url"
            className="w-full rounded-[var(--radius-lg)] border border-[rgba(124,63,44,0.14)] bg-white/90 px-4 py-3 text-sm text-[color:var(--text)] outline-none disabled:cursor-not-allowed disabled:opacity-60"
          />

          <p className="mt-2 text-xs leading-5 text-[color:var(--text-muted)]">
            Hãy chọn đúng nền tảng trước khi dán
            link để Vaffiliate tạo link hoàn tiền
            chính xác.
          </p>
          ...
          {error ? (
            <p
              aria-live="polite"
              className="mt-2 text-sm font-medium text-[color:var(--warning)]"
            >
              {error}
            </p>
          ) : null}
        ...
        <button
          ...
          disabled={isCreatePending}
          onClick={check}
          className="mt-4 w-full rounded-[var(--radius-lg)] bg-[color:var(--brand)] px-4 py-3 text-sm font-semibold text-white shadow-[var(--shadow-sm)] disabled:cursor-not-allowed disabled:opacity-60"
        >
          Kiểm tra hoàn tiền
        </button>
      </div>
        ...
        <div className="mt-4 rounded-[var(--radius-xl)] border border-[color:var(--line)] bg-[rgba(255,248,242,0.92)] p-5 shadow-[var(--shadow-sm)]">
          ...
          <div className="mt-3 grid gap-2 text-sm">
            ...
            <div className="flex justify-between gap-3">
              ...
              <span className="font-semibold text-[color:var(--text)]">
                Sau khi đơn được đối soát và sàn
                xác nhận
              </span>
            </div>
          </div>
          ...
          >
            ...
            <button
              type="submit"
              disabled={isCreatePending}
              className="w-full rounded-[var(--radius-lg)] bg-[color:var(--brand)] px-4 py-3 text-sm font-semibold text-white shadow-[var(--shadow-sm)] disabled:cursor-not-allowed disabled:opacity-60"
            >
              {isCreatePending
                ? "Đang tạo link..."
                : "Tạo link hoàn tiền"}
        ...
        <div className="fixed inset-0 z-50 bg-[rgba(36,24,21,0.44)] backdrop-blur-[2px]">
          ...
          <div className="absolute inset-x-0 bottom-0 rounded-t-[calc(var(--radius-2xl)+0.1rem)] border border-[rgba(124,63,44,0.12)] bg-[linear-gradient(180deg,rgba(255,252,249,0.98),rgba(248,238,231,0.98))] px-4 pb-[calc(1rem+env(safe-area-inset-bottom))] pt-4 shadow-[var(--shadow-lg)] md:left-1/2 md:top-1/2 md:bottom-auto md:w-full md:max-w-[560px] md:-translate-x-1/2 md:-translate-y-1/2 md:rounded-[calc(var(--radius-2xl)+0.1rem)] md:px-6 md:pb-6 md:pt-6">
            ...
            <div className="mt-4 rounded-[var(--radius-lg)] border border-[rgba(124,63,44,0.1)] bg-[rgba(255,250,246,0.82)] p-4">
              ...
              <p className="mt-1 break-all text-sm text-[color:var(--text)]">
                {
                  actionState.trackingLink
                    .destinationUrl
                }
              ...
              <div className="mt-1 break-all rounded-[var(--radius-lg)] border border-[rgba(124,63,44,0.12)] bg-white/90 px-4 py-3 text-sm leading-6 text-[color:var(--text)]">
                {trackingUrl}
              </div>
            </div>
            ...
            {copyOk ? (
              <p
                aria-live="polite"
                className="mt-3 text-sm font-medium text-[color:var(--brand-strong)]"
              >
            ...
            <div className="mt-4 rounded-[var(--radius-xl)] border border-dashed border-[rgba(124,63,44,0.14)] bg-[rgba(255,250,246,0.86)] p-4">
              ...
              <div className="mt-3 flex items-center gap-4">
                ...
                <p className="text-sm leading-6 text-[color:var(--text-muted)]">
                  QR sẽ được tạo từ link hoàn
                  tiền này để bạn mở trên điện
                  thoại.
                </p>
              </div>
            </div>
            ...
            <p className="mt-4 rounded-[var(--radius-lg)] border border-[rgba(124,63,44,0.08)] bg-[rgba(255,250,246,0.78)] px-4 py-3 text-xs leading-5 text-[color:var(--text-muted)] shadow-[var(--shadow-sm)]">
              Hãy mua hàng qua link vừa tạo và
              hạn chế chuyển sang link khác trước
              khi thanh toán để đơn có cơ hội
              được ghi nhận.
            </p>
          </div>
        </div>
